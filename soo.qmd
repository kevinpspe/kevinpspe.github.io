...
These issues occur when the confounder **changes** in value. So what if we hold the confounders constant? Let us assume that the confounder, smartness, has two categories: smart and dumb. Within each category, we calculate the causal effect:
...
Within both of these categories, the confounder is constant. Thus, within each category, correlation is equal to causation, as there is no selection bias. Thus, we can average them to get a final causal effect:
